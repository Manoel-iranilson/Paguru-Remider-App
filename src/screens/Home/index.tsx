...
import { AlertTask, Body, Btn, BtnAdd, BtnText, Container, Footer, TextBody, TitleBody } from './styles'
...
const Home = () => {
    ...
    const renderItem = ({ item }: any) => {
        ...
        if (item.task?.length === 0) {
            return <View style={{ alignItems: 'center' }}><TextBody > Registre uma atividade</TextBody ></View>
        } else {
            return (
                        ...
                        <Swipeable
                            renderRightActions={() =>
                                <Btn onPress={() => onDelete(item, el.id)} >
                                    <LinearGradient start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} colors={['#FFDEAD', '#fff']}>
                                        <BtnText style={{ color: 'red' }}>Deletar</BtnText>
                                    </LinearGradient>
                                </Btn>}
                            renderLeftActions={() =>
                                <Btn onPress={() => onCompleted(item, el.id)}>
                                    <LinearGradient start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} colors={['#fff', '#8FBC8F']}>
                                        <BtnText style={{ color: 'green' }}>{el.isCompleted ? 'Ativar' : 'Concluir'}</BtnText>
                                    </LinearGradient>
                                </Btn>}
                        >
    ...
    return (
        <Container>
            ...
            <Modal visible={showSettings} transparent={true} onRequestClose={() => setShowSettings(false)} >
            ...
            <Footer>
                ...
                <TouchableOpacity activeOpacity={1} onPress={() => navigation.navigate("NewTask")}>
                    <BtnAdd start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} colors={['#f39466', '#f94d73']}>
